    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.teams.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('governorates') ? 'is-invalid' : '' }}" name="governorates[]" id="governorates" multiple>
                    @foreach($governorates as $id => $governorates)
                        <option value="{{ $id }}" {{ in_array($id, old('governorates', [])) ? 'selected' : '' }}>{{ $governorates }}</option>
                    @endforeach
                </select>
